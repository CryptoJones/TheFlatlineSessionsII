...
const VOID_TRACKS := [
	...
	"cyberspace",
	"ice_combat",
	"ch01_smooth_running_gun",
	"ch02_bobby_pulls_a_wilson",
	"ch03_vireks_garden",
	"ch04_on_site",
	"ch05_the_projects",
	"ch06_names_of_the_dead",
	"ch07_the_squirrel_wood",
	"ch08_count_binary_interrupt",
	"ch09_the_boxmaker",
]
const TRACK_TITLES := {
	"title": "THE FLATLINE SESSIONS II",
	"streets": "Sprawl Weather",
	"shops": "Black Clinics Take Credit",
	"cyberspace": "Count Binary Interrupt",
	"ice_combat": "ICE Has Teeth",
	"ch01_smooth_running_gun": "Smooth-Running Gun",
	"ch02_bobby_pulls_a_wilson": "Bobby Pulls a Wilson",
	"ch03_vireks_garden": "Virek's Garden",
	"ch04_on_site": "On Site",
	"ch05_the_projects": "The Projects",
	"ch06_names_of_the_dead": "Names of the Dead",
	"ch07_the_squirrel_wood": "The Squirrel Wood",
	"ch08_count_binary_interrupt": "Count Binary Interrupt",
	"ch09_the_boxmaker": "The Boxmaker",
}
...
func _bar_height() -> int:
	...
	for b in _button_bar.get_children():
		...
		var w: float = b.get_theme_font("font").get_string_size(b.text, HORIZONTAL_ALIGNMENT_LEFT, -1, fs).x \
			+ b.get_theme_stylebox("normal").get_minimum_size().x + 2
		row_h = maxi(row_h, ceili(b.get_theme_font("font").get_height(fs) + b.get_theme_stylebox("normal").get_minimum_size().y) + 4)
...
func _build_endgame_layer() -> void:
	# A composed finale card, like the title screen: its type is display type and
	# keeps its size whatever the reader's Text Size setting.
