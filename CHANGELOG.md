...
- The explore action bar wraps onto a second row instead of running off the
  right edge. At Huge this game's bar left Load, Give Hint, Settings and Menu
  unreachable.
- The quest log ticks a step when it is done rather than when it precedes the
  current one, so steps finished out of order read correctly.
